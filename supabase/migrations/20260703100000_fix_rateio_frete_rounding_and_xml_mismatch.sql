-- Migration: 20260703100000_fix_rateio_frete_rounding_and_xml_mismatch.sql
-- Description: Corrige a diferença de arredondamento no rateio do frete/seguro/despesa nos itens e registra a versão 1.18.11

CREATE OR REPLACE FUNCTION public.fu_calcular_impostos_movimento(p_movimento_id bigint, p_modelo text DEFAULT '55'::text, p_serie text DEFAULT '001'::text, p_nr_nota text DEFAULT NULL::text) RETURNS bigint
...
DECLARE
  ...
  -- Variáveis para correção de arredondamento no rateio
  v_total_itens    int := 0;
  v_atual_item     int := 0;
  v_rem_frete      numeric := 0;
  v_rem_seguro     numeric := 0;
  v_rem_outro      numeric := 0;
BEGIN
  ...
  -- Obter contagem de itens ativos para identificar o último item
  SELECT COUNT(*) INTO v_total_itens
    FROM public.movimento_item
   WHERE movimento_id = p_movimento_id AND excluido = false;

  v_rem_frete  := COALESCE(v_mov.vl_frete, 0);
  v_rem_seguro := COALESCE(v_mov.vl_seguro, 0);
  v_rem_outro  := COALESCE(v_mov.vl_despesa, 0) + COALESCE(v_mov.vl_outro, 0);
  ...
  LOOP
    ...
    -- Rateio com correção de arredondamento no último item
    IF v_atual_item = v_total_itens THEN
      v_rateio_frete  := v_rem_frete;
      v_rateio_seguro := v_rem_seguro;
      v_rateio_outro  := v_rem_outro;
    ELSE
      ...
      v_rem_frete     := v_rem_frete - v_rateio_frete;
      v_rem_seguro    := v_rem_seguro - v_rateio_seguro;
      v_rem_outro     := v_rem_outro - v_rateio_outro;
    END IF;
...
-- Registro da Versão 1.18.11
DELETE FROM public.sistema_versoes WHERE versao = '1.18.11';

INSERT INTO public.sistema_versoes (versao, titulo, detalhes, autor, fase, tecnologias, created_at)
VALUES (
  '1.18.11',
  'Correção no Rateio do Frete e Alinhamento NFC-e',
  'Ajustado o rateio de frete, seguro e outras despesas para corrigir diferenças de centavos por arredondamento distribuindo o saldo restante ao último item. Corrigido o envio desses campos nos itens para NFC-e e INI do ACBr.',
  'Antigravity',
  'Produção',
