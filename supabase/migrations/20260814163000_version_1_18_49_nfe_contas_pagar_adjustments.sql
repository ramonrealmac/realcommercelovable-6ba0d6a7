-- Migration: 20260814163000_version_1_18_49_nfe_contas_pagar_adjustments.sql
-- Description: Versão 1.18.49 - Ajustes no Contas a Pagar, Importação de NF-e, Aba Títulos, Parsing de Duplicatas e Trigger Consolidada.

-- 1. Atualização da Trigger de Consolidação Financeira (evita erro de FK em portador_id)
...
-- 2. Registro da Versão 1.18.49 na tabela public.sistema_versoes
DELETE FROM public.sistema_versoes WHERE versao = '1.18.49';
INSERT INTO public.sistema_versoes (versao, titulo, detalhes, autor, fase, tecnologias, created_at)
VALUES (
  '1.18.49',
  'Ajustes em Contas a Pagar, Duplicatas XML, Aba Títulos da NF-e e Trigger Consolidada',
  '• NF-e Entrada: Remoção das colunas tributárias da grid e criação da nova aba Títulos da NF-e para visualização/previsão das parcelas.' || chr(10) ||
  '• Duplicatas XML: Extração do número da duplicata (<nDup>) e gravação/exibição da parcela exata na grid (ex: 25091 / 1, 25091 / 2).' || chr(10) ||
  '• Estorno de Escrituração: Exclusão física (hard delete) dos títulos no Contas a Pagar ao estornar para eliminar lançamentos cancelados residuais.' || chr(10) ||
  '• Gerenciador de Pagamentos: Inclusão do campo parcela na query da view, exibição da coluna Título / Parcela e clique duplo abrindo a aba Títulos Recebidos na parcela exata.' || chr(10) ||
  '• Tratamento de Erros e FKs: Melhoria na captura de mensagens do Supabase (removendo [object Object]), sanitização de IDs nulos e atualização da trigger fu_financeiro_baixa_consolidacao com NULLIF(portador_id, 0).',
  'AI Antigravity',
  'Desenvolvimento',
  ARRAY['PostgreSQL', 'Supabase', 'React', 'TypeScript'],
  now()
);
