-- Migration: 20260818190000_fix_movimento_pagamento_vl_parcelas.sql
-- Description: Updates fu_pdv_registrar_recebimento_venda to save vl_parcelas in movimento_pagamento and recalculates n_parcelas and vl_parcelas for all existing records.

-- 1. Update fu_pdv_registrar_recebimento_venda function
...
DECLARE
  ...
  -- Variáveis adicionais para geração do financeiro e parcelas
...
BEGIN
  ...
  -- 4. Grava movimento_pagamento (COM N_PARCELAS E VL_PARCELAS DEDUZIDOS DA CONDICAO OU PAYLOAD)
  FOR v_pag_row IN SELECT * FROM jsonb_array_elements(_pagamentos) LOOP
...
-- 2. Recalcula n_parcelas e vl_parcelas para TODOS os pedidos em movimento_pagamento
UPDATE public.movimento_pagamento mp
SET 
  n_parcelas = calc.n_parc,
  vl_parcelas = ROUND(COALESCE(mp.vl_pagamento, 0) / calc.n_parc, 2)
FROM (
  SELECT 
    m.movimento_pagamento_id,
    CASE 
      WHEN m.n_parcelas IS NOT NULL AND m.n_parcelas > 1 THEN m.n_parcelas
      WHEN c.tipo_prazo = 'V' AND (
        COALESCE(c.prazo_1,0)+COALESCE(c.prazo_2,0)+COALESCE(c.prazo_3,0)+COALESCE(c.prazo_4,0)+
        COALESCE(c.prazo_5,0)+COALESCE(c.prazo_6,0)+COALESCE(c.prazo_7,0)+COALESCE(c.prazo_8,0)+
        COALESCE(c.prazo_9,0)+COALESCE(c.prazo_10,0)+COALESCE(c.prazo_11,0)+COALESCE(c.prazo_12,0)
      ) > 0 THEN (
        (CASE WHEN COALESCE(c.prazo_1, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_2, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_3, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_4, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_5, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_6, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_7, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_8, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_9, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_10, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_11, 0) > 0 THEN 1 ELSE 0 END) +
        (CASE WHEN COALESCE(c.prazo_12, 0) > 0 THEN 1 ELSE 0 END)
      )
      ELSE COALESCE(NULLIF(c.qtd_parcelas, 0), 1)
    END AS n_parc
  FROM public.movimento_pagamento m
  LEFT JOIN public.condicao_pagamento c ON c.condicao_id = m.condicao_id
) AS calc
WHERE mp.movimento_pagamento_id = calc.movimento_pagamento_id;
