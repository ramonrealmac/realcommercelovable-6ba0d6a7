-- Migration: 20260915113000_update_fu_gerar_financeiro_movimento_ignore_credito.sql
-- Description: Update fu_gerar_financeiro_movimento to skip generating pending financeiro titles for customer credit payments.
...
BEGIN
  ...
  LOOP
    ...
    -- Se for Crédito de Cliente / Utilizar Crédito, não gera título a receber pendente (pois já foi quitado do saldo do cliente)
    IF LOWER(COALESCE(v_cond.descricao, '')) LIKE '%crédito%' OR LOWER(COALESCE(v_cond.descricao, '')) LIKE '%credito%' THEN
      CONTINUE;
    END IF;
