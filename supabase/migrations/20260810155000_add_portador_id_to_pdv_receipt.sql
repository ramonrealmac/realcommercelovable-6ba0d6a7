-- Migration: 20260810155000_add_portador_id_to_pdv_receipt.sql
-- Description: Updates fu_pdv_registrar_recebimento_venda to parse portador_id and save it in movimento_pagamento and financeiro tables. Registers version 1.18.44.
...
BEGIN
  ...
  FOR v_pag_row IN SELECT * FROM jsonb_array_elements(_pagamentos) LOOP
    ...
    v_portador_id := NULLIF(v_pag_row->>'portador_id', '')::bigint;
    ...
    INSERT INTO movimento_pagamento (
      ...
      v_n_parcelas, now(), v_portador_id
    );
  END LOOP;
  ...
  FOR v_pag_row IN SELECT * FROM jsonb_array_elements(_pagamentos) LOOP
    ...
    IF v_pag_dinheiro_ajustado > 0 THEN
      INSERT INTO caixa_movimento_item (
        ...
        v_pag_dinheiro_ajustado / v_n_parcelas, -- Recalcula parcela
    ...
    IF v_meio_pagamento_id IN (3, 5, 14, 15, 91) THEN
      ...
      FOR v_i IN 1..v_n_parcelas LOOP
        ...
        IF v_cond IS NOT NULL AND v_cond.tipo_prazo = 'F' THEN
          ...
          -- Se o prazo resultante for 0 mas tivermos um intervalo > 0, usamos o intervalo
          IF v_prazo = 0 AND COALESCE(v_cond.intervalo, 0) > 0 THEN
            v_prazo := v_cond.intervalo * v_i;
          END IF;
        ELSE
...
-- Register version 1.18.44
DELETE FROM public.sistema_versoes WHERE versao = '1.18.44';
INSERT INTO public.sistema_versoes (versao, titulo, detalhes, autor, fase, tecnologias)
VALUES (
  '1.18.44',
  'Persistência de Portador no Recebimento do Caixa',
  'PDV: Atualizada a função fu_pdv_registrar_recebimento_venda para obter e gravar o portador_id na tabela movimento_pagamento e na geração do financeiro.',
  'AI Antigravity',
  'Desenvolvimento',
