-- Migration: 20260811130000_add_tipo_antecipacao_to_pdv_recebimento.sql
-- Description: Updates fu_pdv_registrar_recebimento_venda to process credit card payment antecipação rules (SEM ANTECIPAÇÃO, AUTOMÁTICA, AVULSA).
-- Automatic mode creates 1 single title in financeiro with status = 'B' (Baixado), vl_despesa = operadora fee + antecipacao fee, vl_pago = net amount, inserts into financeiro_baixa and synchronizes with financeiro_consolidado.
  ...
  FOR v_pag_row IN SELECT * FROM jsonb_array_elements(_pagamentos) LOOP
    ...
    IF v_meio_pagamento_id IN (3, 5, 14, 15, 91) THEN
      ...
      IF v_meio_pagamento_id = 3 AND UPPER(v_tipo_antecipacao) = 'AUTOMÁTICA' THEN
        ...
        -- Total Despesa (Operadora + Antecipação) e Valor Líquido Pago
        v_vl_despesa_total := v_vl_despesa_op + v_vl_despesa_ant;
        v_vl_liquido := GREATEST(0, v_vl_bruto - v_vl_despesa_total);

        -- Insere 1 parcela única na tabela financeiro já com status = 'B' (BAIXADO)
