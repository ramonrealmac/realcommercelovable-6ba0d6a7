-- Migration: 20260811151500_fix_financeiro_liquido_and_baixa.sql
-- Description: Fixes financeiro title payment calculations. Total Liquido = (vl_titulo - vl_desconto - vl_despesa + juros + multa). Set vl_pago = vl_liquido on baixas with despesas, keeping status B (BAIXADO) when (vl_pago + vl_desconto + vl_despesa) >= vl_titulo.

-- 1. Update fu_financeiro_baixa_consolidacao
...
BEGIN
    ...
    -- Define o novo status do título (B = Baixado, A = Aberto) considerando (vl_pago + vl_desconto + vl_despesa) >= vl_titulo
    ...
    -- 3. Atualizar o Título Financeiro (vl_pago é o valor pago efetivo líquido)
...
-- 2. Update fu_pdv_registrar_recebimento_venda
...
BEGIN
  ...
  -- 8b. Gerar parcelas/títulos no financeiro
    ...
    IF v_meio_pagamento_id IN (3, 5, 14, 15, 91) THEN
      ...
      IF v_meio_pagamento_id = 3 AND UPPER(v_tipo_antecipacao) = 'AUTOMÁTICA' THEN
        ...
        INSERT INTO public.financeiro (
          ...
          v_vl_bruto,
          0,
          v_vl_liquido, -- Valor Pago líquido = 1456.00
          0,
          v_vl_despesa_total, -- Despesa = 144.00
        ...
        INSERT INTO public.financeiro_baixa (
          ...
          (COALESCE(v_mov.nr_movimento::text, _movimento_id::text) || '-1'),
          _dt_movimento,
          v_vl_liquido, -- Valor Pago na Baixa = 1456.00
          v_vl_despesa_total, -- Despesa = 144.00
          0, 0,
          'CARTAO',
          3,
...
-- 3. Update existing title records in database where status = 'B' and despesa > 0
UPDATE public.financeiro
SET vl_pago = (vl_titulo - COALESCE(vl_desconto, 0) - COALESCE(vl_despesa, 0) + COALESCE(vl_adicional, 0))
WHERE status = 'B' 
  AND COALESCE(vl_despesa, 0) > 0 
  AND vl_pago > (vl_titulo - COALESCE(vl_desconto, 0) - COALESCE(vl_despesa, 0));

UPDATE public.financeiro_baixa fb
SET vl_pago = f.vl_pago
FROM public.financeiro f
WHERE fb.financeiro_id = f.financeiro_id
  AND f.status = 'B'
  AND COALESCE(f.vl_despesa, 0) > 0
  AND fb.vl_pago > f.vl_pago;
