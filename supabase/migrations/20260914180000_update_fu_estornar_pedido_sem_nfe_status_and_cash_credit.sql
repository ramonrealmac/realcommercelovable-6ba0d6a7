-- Migration: 20260914180000_update_fu_estornar_pedido_sem_nfe_status_and_cash_credit.sql
-- Description: Atualiza a RPC fu_estornar_pedido_sem_nfe para alterar o st_pedido para 'C' (Cancelado/Estornado) ao realizar estorno total e limitar a geração de crédito/saída em dinheiro estritamente ao valor em dinheiro recebido.
...
BEGIN
  ...
  IF v_mov.st_pedido IN ('C', 'E') THEN
    RETURN jsonb_build_object('error', 'Este pedido já foi cancelado ou totalmente estornado.');
  END IF;
  ...
  -- 6. Atualiza st_pedido para 'C' (Cancelado/Estornado) se todos os itens foram devolvidos
  ...
  RETURN jsonb_build_object(
    ...
    'vl_dinheiro_estorno', v_vl_dinheiro_estorno,
    'opcao_dinheiro', _opcao_dinheiro,
    'st_pedido', CASE WHEN v_pendente_qtd <= 0 THEN 'C' ELSE v_mov.st_pedido END
  );
