-- Migration: 20260703110000_add_ind_pres_to_cabecalho.sql
-- Description: Adiciona coluna ind_pres em fiscal_nfe_cabecalho e atualiza fu_calcular_impostos_movimento para popula-lo

ALTER TABLE public.fiscal_nfe_cabecalho ADD COLUMN IF NOT EXISTS ind_pres character varying(1);
...
DECLARE
  ...
  -- Variável para presença
  v_ind_pres       character varying(1);
BEGIN
  ...
  -- Determinar ind_pres
  v_ind_pres := COALESCE(
    NULLIF(v_mov.ind_pres, ''),
    CASE WHEN COALESCE(v_mov.vl_frete,0) > 0 AND (v_mov.logradouro_entrega IS NOT NULL OR v_mov.cep_entrega IS NOT NULL) THEN '4'
         ELSE '1'
    END
  );
