-- Migration: 20260915130000_update_fu_estornar_pedido_sem_nfe_credito_and_electronic.sql
-- Description: Update fu_estornar_pedido_sem_nfe to treat customer credit store payments as money/flexible refund, and automatically credit electronic payments (Pix, Cards, Checks, Transfers) to customer credit.
...
BEGIN
  ...
  -- 3. Identifica se o recebimento original envolveu DINHEIRO ou CRÉDITO DO CLIENTE e soma os valores
  FOR v_rec_row IN
    SELECT 
  ...
  -- 5a. Tratamento do valor em Dinheiro / Crédito do Cliente (Espécie ou Crédito)
  IF v_meio_dinheiro AND v_vl_dinheiro_estorno > 0 THEN
    IF _opcao_dinheiro IS NULL OR _opcao_dinheiro NOT IN ('ESPECIE', 'CREDITO') THEN
      RETURN jsonb_build_object('error', 'Para recebimentos em dinheiro ou crédito em loja, é obrigatório selecionar entre DEVOLVER EM ESPÉCIE ou GERAR CRÉDITO PARA O CLIENTE.');
    END IF;
    ...
    ELSIF _opcao_dinheiro = 'CREDITO' THEN
      -- Gera CRÉDITO PARA O CLIENTE no extrato de créditos
