-- Migration: 20260916163000_update_estorno_sem_nfe_check_paid_financeiro.sql
-- Description: Atualiza fu_estornar_pedido_sem_nfe para identificar parcelas baixadas na tabela financeiro (status = 'B' / vl_pago > 0) permitindo escolher entre DEVOLVER EM ESPÉCIE ou GERAR CRÉDITO PARA O CLIENTE.
...
BEGIN
  ...
  -- 3. Identifica se o recebimento original envolveu DINHEIRO, CRÉDITO DO CLIENTE ou TITULOS BAIXADOS
  FOR v_rec_row IN
    SELECT 
  ...
  LOOP
    IF v_rec_row.meio_pagamento_id = 1 
       OR UPPER(COALESCE(v_rec_row.soma_vl_caixa, 'N')) = 'S' 
       OR LOWER(COALESCE(v_rec_row.cond_desc, '')) LIKE '%crédito%' 
       OR LOWER(COALESCE(v_rec_row.cond_desc, '')) LIKE '%credito%' THEN
  ...
  -- Se ainda não achou valor, verifica se há parcelas BAIXADAS na tabela financeiro (status = 'B')
  IF v_vl_dinheiro_original = 0 THEN
    FOR v_rec_row IN
      SELECT COALESCE(vl_pago, vl_titulo, 0) as valor
  ...
  -- 5a. Tratamento do valor em Dinheiro / Crédito do Cliente / Título Baixado
  IF v_meio_dinheiro AND v_vl_dinheiro_estorno > 0 THEN
    IF _opcao_dinheiro IS NULL OR _opcao_dinheiro NOT IN ('ESPECIE', 'CREDITO') THEN
      RETURN jsonb_build_object('error', 'Para recebimentos/parcelas baixadas, é obrigatório selecionar entre DEVOLVER EM ESPÉCIE ou GERAR CRÉDITO PARA O CLIENTE.');
    END IF;
  ...
  -- 5b. Tratamento automático de pagamentos eletrônicos (PIX, Cartão, Cheque, Transferência) -> Crédito do Cliente
  IF v_vl_outros_estorno > 0 AND v_cadastro_id IS NOT NULL THEN
    PERFORM public.fu_registrar_movimento_credito(
      _empresa_id            := _empresa_id,
      _cadastro_id           := v_cadastro_id,
      _tp_parceiro           := 'CLIENTE',
      _tp_movimento          := 'C',
      _vl_movimento          := v_vl_outros_estorno,
      _origem_movimento      := 'ESTORNO_PEDIDO_SEM_NFE',
      _historico             := 'CRÉDITO DO CLIENTE GERADO POR ESTORNO ELETRÔNICO (R$ ' || to_char(v_vl_outros_estorno, 'FM999G999G990D00') || ') DO PEDIDO ' || COALESCE(v_mov.nr_movimento::text, _movimento_id::text),
      _usuario_id            := _usuario_id,
      _movimento_id          := _movimento_id
    );
  END IF;
