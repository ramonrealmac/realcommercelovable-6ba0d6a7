...
const DevolucaoNfeEntradaForm: React.FC<DevolucaoNfeEntradaFormProps> = ({ initialNfeId }) => {
  ...
  const [XNatOp, setXNatOp] = useState("DEVOLUCAO DE COMPRA PARA REFINAMENTO E/OU MERCADORIA");
  ...
  const buscarNotas = async () => {
    ...
    try {
      let q = db.from("fiscal_nfe_cabecalho")
        .select("nfe_cabecalho_id,nr_nota,serie,dt_emissao,dt_entrada,vl_total_nf,chave_nfe,cadastro_id,st_nf,modelo,tp_nf,fin_nfe,cadastro:cadastro_id(razao_social,cnpj)")
        .eq("empresa_id", XEmpresaId)
        .eq("tp_nf", 0) // Apenas notas recebidas de entrada
        .neq("fin_nfe", 4) // Não carregar notas de devolução (tipo 4)
  ...
  const gerarDevolucao = async () => {
    ...
    if (itensValidos.some(it => !it.deposito_id)) { toast.error("Selecione o depósito de saída para todos os itens a devolver."); return; }
          ...
          <div className="flex flex-col gap-3 h-full">
            ...
            <div className="border border-border rounded-lg overflow-hidden flex-1 flex flex-col">
              <div className="grid grid-cols-12 gap-2 px-3 py-2 bg-muted/50 text-[10px] font-bold uppercase text-muted-foreground border-b">
                ...
                <div className="col-span-2 text-left">Depósito Saída</div>
