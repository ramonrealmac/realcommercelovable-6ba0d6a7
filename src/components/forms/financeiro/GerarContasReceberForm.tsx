  ...
  useEffect(() => {
    (async () => {
      ...
      const [{ data: cli }, { data: mp }, { data: po }, { data: pl }] = await Promise.all([
        supabase.from("cadastro").select("cadastro_id, nome_fantasia, razao_social").eq("excluido", false).order("nome_fantasia"),
        supabase.from("meio_pagamento").select("codigo, descricao").order("descricao"),
        supabase.from("portador").select("portador_id, nome, banco_id").order("nome"),
        supabase.from("plano_conta").select("plano_conta_id, nome").eq("tp_conta", "A").eq("tp_natureza", "R").eq("excluido", false).order("nome"),
      ]);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
