...
const MdfDescarregaTab: React.FC<IProps> = ({ mdfManifestoId, empresaId, podeEditar }) => {
  ...
  return (
    <div className="space-y-4 p-2">
      ...
      <table className="w-full text-sm border-collapse">
        ...
        <tbody>
          {rows.map(r => (
            <tr key={r.mdf_descarrega_id} className="hover:bg-accent/30">
              ...
              {podeEditar && (
                <td className="px-3 py-1.5 border border-border text-center">
                  <button onClick={() => handleRemove(r.mdf_descarrega_id)} className="text-destructive hover:text-destructive/80">
          ...
          {!rows.length && <tr><td colSpan={3} className="px-3 py-4 text-center text-muted-foreground text-sm">Nenhuma cidade adicionada.</td></tr>}
        </tbody>
      </table>
    </div>
  );
