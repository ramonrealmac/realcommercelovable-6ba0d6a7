    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader className="bg-topbar p-4 rounded-t-lg -mx-6 -mt-6 mb-4">
          <DialogTitle className="text-topbar-foreground opacity-70 uppercase tracking-wider text-center">Funções do Caixa</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex justify-end pt-2 border-t border-border">
          <button onClick={onClose} className="text-sm px-4 py-1.5 rounded border border-border hover:bg-accent flex items-center gap-2">
            <X size={16} className="text-rose-500" /> Fechar
          </button>
        </div>
      </DialogContent>
    </Dialog>
