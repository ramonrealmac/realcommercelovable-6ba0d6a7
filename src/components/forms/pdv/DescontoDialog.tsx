...
const DescontoDialog: React.FC<IProps> = ({ open, subtotal, descontoAtual, percAtual, onClose, onAplicar }) => {
  ...
  useEffect(() => {
    if (!open) return;
    if (percAtual && percAtual > 0) { setXTipo("P"); setXValor(String(percAtual).replace(".", ",")); }
    else if (descontoAtual && descontoAtual > 0) { setXTipo("V"); setXValor(String(descontoAtual).replace(".", ",")); }
    else { setXTipo("P"); setXValor(""); }
