...
const MonitorFiscalTestForm: React.FC = () => {
  ...
  const [command, setCommand] = useState("ACBr.Status()");
  ...
  const handleSend = async () => {
    ...
    try {
      // Se estiver em modo bridge, a URL é a da ponte
      // Se estiver em modo direct, a URL é a do ACBrMonitor HTTP
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card className="md:col-span-2 shadow-lg border-primary/20">
          ...
          <CardContent className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              <Label>Resposta do Monitor</Label>
              <Textarea 
                readOnly 
                value={response} 
            ...
            <div className="flex gap-2">
              {["ACBr.Status()", "NFE.Ativo()", "BOLETO.Ativo()"].map(cmd => (
                <Button 
                  key={cmd} 
                  variant="outline" 
                  size="sm" 
