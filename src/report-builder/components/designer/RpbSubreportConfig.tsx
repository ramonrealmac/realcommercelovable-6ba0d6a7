...
const CANVAS_SCALE = 3.2; // px por mm (idêntico ao RpbCanvas.tsx)
...
const UnifiedSubreportSheetCanvas: React.FC<{
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between p-2.5 bg-card border border-border rounded-lg shadow-sm">
      ...
      <div className="flex justify-center bg-gray-100/80 p-4 rounded-xl border border-border">
        <div
          className="shadow-2xl border border-gray-300 bg-white rounded overflow-hidden select-none"
                ...
                headerComps.map((c, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={e => { e.stopPropagation(); setActiveSubBand('header'); setSelectedCompId(c.id); }}
                      onMouseDown={e => {
                        e.stopPropagation();
                        e.preventDefault();
                ...
                detailComps.map((c, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={e => { e.stopPropagation(); setActiveSubBand('detail'); setSelectedCompId(c.id); }}
                      onMouseDown={e => {
                        e.stopPropagation();
                        e.preventDefault();
              ...
              {footerComps.map((c, idx) => {
                ...
                return (
                  <div
                    ...
                    onClick={e => { e.stopPropagation(); setActiveSubBand('footer'); setSelectedCompId(c.id); }}
                    onMouseDown={e => {
                      e.stopPropagation();
                      e.preventDefault();
      ...
      {draft.tipoLayout !== 'custom' && activeSubBand === 'detail' && !selectedComp ? (
        <div className="space-y-3 p-4 border border-border rounded-lg bg-card shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold">Colunas da Tabela do Sub-Relatório</p>
              <p className="text-[11px] text-muted-foreground mt-0.5">Arraste a alça para reordenar a ordem das colunas.</p>
            </div>
            <button onClick={handleDetectCols} disabled={detecting}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded border border-border hover:bg-secondary disabled:opacity-50 font-semibold">
              {detecting ? <Loader2 size={13} className="animate-spin" /> : <RefreshCw size={13} />}
              Re-detectar Colunas do SQL
            </button>
          </div>
          ...
          {draft.columns.length === 0 && (
            <div className="text-center py-6 border-2 border-dashed border-border rounded-lg text-muted-foreground">
              ...
              <p className="text-xs mt-1">Configure a Query na aba SQL e clique em "Detectar Colunas".</p>
            </div>
          )}
            ...
            <div className="space-y-1 max-h-48 overflow-auto">
              ...
              {draft.columns.map((col, i) => (
                ...
                >
                  ...
                  <input type="number" className={inputCls} value={col.w} min={5} max={210}
                    onChange={e => updateCol(i, { w: Number(e.target.value) })} />
                  <select className={inputCls} value={col.align} onChange={e => updateCol(i, { align: e.target.value as any })}>
                    <option value="left">Esquerda</option>
                    <option value="center">Centro</option>
                    <option value="right">Direita</option>
                  </select>
                  <select className={inputCls} value={col.format} onChange={e => updateCol(i, { format: e.target.value as any })}>
                  ...
                  <select className={inputCls} value={col.totalType || 'none'} onChange={e => updateCol(i, { totalType: e.target.value as any })}>
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.7)' }}>
      <div
        ...
        style={{ width: '860px', maxWidth: '95vw', maxHeight: '94vh' }}
        onClick={e => e.stopPropagation()}
      >
        ...
        <div className="flex-1 overflow-auto p-5">
          ...
          {/* ── Aba Formatação por Bandas (Folha Única Canvas Contínua) ─────────────────────── */}
